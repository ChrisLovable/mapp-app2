 ...
 */
export async function convertMultipleWebmToMp3(files: string[], outputDir: string): Promise<void> {
 ...
 * @returns Promise that resolves when conversion is complete
 */
export function convertWebmToMp3WithOptions(
  inputPath: string, 
  outputPath: string, 
  options: {
    bitrate?: number;
 ...
 */
export function getAudioInfo(filePath: string): Promise<any> {
  return new Promise((resolve, reject) => {
