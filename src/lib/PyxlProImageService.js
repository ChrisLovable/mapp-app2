...
export class PyxlProImageService {
    ...
    async generateImage(options) {
        const { prompt, referenceImage, styleInfo, strength = 0.7, guidance_scale = 7.5 } = options;
        if (!this.apiKey) {
            return {
                success: false,
        ...
        try {
            ...
            if (referenceImage) {
                ...
                if (!response.ok) {
                    ...
                    apiUsageTracker.trackOpenAIUsage(endpoint, 'pyxlpro', 0, 0, 'Image Transformation', false, errorMessage);
                    return {
                        success: false,
                ...
                apiUsageTracker.trackOpenAIUsage(endpoint, 'pyxlpro', usage?.prompt_tokens || 0, usage?.completion_tokens || 0, 'Image Transformation', true);
                return {
                    success: true,
                    imageUrl,
                    usage: {
                ...
                if (!response.ok) {
                    ...
                    console.log('Error Data:', JSON.stringify(errorData, null, 2));
                    // Track failed API usage
                    apiUsageTracker.trackOpenAIUsage(endpoint, 'pyxlpro', 0, 0, 'Image Generation', false, errorMessage);
                    return {
                        success: false,
                        error: `PyxlPro API error: ${errorMessage}`
                    };
                ...
                console.log('Generated Image URL:', imageUrl);
                // Track successful API usage
                apiUsageTracker.trackOpenAIUsage(endpoint, 'pyxlpro', usage?.prompt_tokens || 0, usage?.completion_tokens || 0, 'Image Generation', true);
                return {
                    success: true,
                    imageUrl,
                    usage: {
            ...
            // Track failed API usage
            apiUsageTracker.trackOpenAIUsage('https://api.pyxlpro.com/v1/image-to-image', 'pyxlpro', 0, 0, 'Image Generation', false, error instanceof Error ? error.message : 'Unknown error');
            return {
                success: false,
