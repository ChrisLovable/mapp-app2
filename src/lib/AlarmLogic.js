...
export function getAlarmTimeFromText(input) {
    try {
...
export function scheduleAlarm(targetTime, onTrigger, alarmId) {
...
export async function showAlarmNotification(title, body) {
    try {
...
export class AlarmManager {
    ...
    setAlarmTriggerCallback(callback) {
        this.onAlarmTrigger = callback;
    }
    // Add a new alarm
    addAlarm(parsedAlarm) {
        const alarmId = `alarm-${this.nextId++}`;
        const timeoutId = scheduleAlarm(parsedAlarm.targetTime, () => this.triggerAlarm(alarmId), alarmId);
        const alarm = {
            id: alarmId,
    ...
    async triggerAlarm(alarmId) {
        ...
        // Show notification
        await showAlarmNotification('⏰ Alarm', alarm.description);
            ...
            if (stored) {
                ...
                alarmsData.forEach((alarmData) => {
                    ...
                    if (alarm.isActive && alarm.targetTime.getTime() > Date.now()) {
                        const timeoutId = scheduleAlarm(alarm.targetTime, () => this.triggerAlarm(alarm.id), alarm.id);
