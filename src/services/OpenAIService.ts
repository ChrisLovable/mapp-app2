...
export class OpenAIService {
  ...
  async getResponse(prompt: string, config?: PromptConfig): Promise<AIResponse> {
    if (!this.apiKey) {
  ...
  async getResponseWithRetry(prompt: string, config?: PromptConfig, maxRetries: number = 3): Promise<AIResponse> {
  ...
  async analyzeExpenseData(prompt: string, expenseData: any): Promise<AIResponse> {
...
Please provide a clear, helpful response based on this data.`;
  ...
  async answerQuickQuestion(question: string, contextData: any): Promise<AIResponse> {
...
Please provide insights about spending patterns, trends, and recommendations.`;
  ...
  async generateInsights(insightData: any): Promise<AIResponse> {
...
Please provide actionable insights and recommendations for better financial management.`;
  ...
  isWithinTokenLimit(prompt: string, maxTokens: number = 4000): boolean {
    const estimatedTokens = this.estimateTokens(prompt);
    return estimatedTokens <= maxTokens;
  }

  // Truncate prompt if it exceeds token limits
  truncatePrompt(prompt: string, maxTokens: number = 4000): string {
