...
export class QuestionProcessor {
  private analytics: ExpenseAnalytics;
  ...
  async processQuestion(context: QuestionContext): Promise<ProcessedQuestion> {
  ...
  private async getRelevantData(userId: string, questionType: QuestionType, filters?: AnalysisFilters) {
    try {
      switch (questionType) {
        case 'vendor':
  ...
  private buildPrompt(question: string, data: any, questionType: QuestionType): string {
...
Focus on totals, averages, and key summary statistics.`;
  ...
  private getMaxTokensForQuestionType(questionType: QuestionType): number {
    switch (questionType) {
      case 'insights':
      case 'forecast':
  ...
  private getTemperatureForQuestionType(questionType: QuestionType): number {
    switch (questionType) {
      case 'insights':
      case 'forecast':
  ...
  async answerQuickQuestion(userId: string, question: string): Promise<string> {
  ...
  async getComprehensiveAnalysis(userId: string, filters?: AnalysisFilters): Promise<ProcessedQuestion> {
  ...
  async querySpecificData(userId: string, question: string, dataType: 'vendors' | 'categories' | 'trends' | 'summary'): Promise<any> {
    try {
      switch (dataType) {
        case 'vendors':
