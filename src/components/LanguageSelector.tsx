...
export default function LanguageSelector({ value, onChange, languages }: Props) {
  return (
    <div className="relative rounded-full overflow-visible">
      <select
        className="glassy-btn neon-grid-btn font-semibold text-[10px] rounded-full text-center p-2 select-none cursor-pointer border-0 transition-all duration-200 active:scale-95 relative overflow-visible w-full max-w-[110px] min-w-[80px]"
        value={value}
        onChange={onChange}
        style={{
          background: '#111',
