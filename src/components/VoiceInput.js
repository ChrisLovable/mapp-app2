...
export default function VoiceInput({ language, onResult }) {
    ...
    return (_jsx("button", { className: `glassy-btn neon-grid-btn p-2 rounded-full shadow-md transition-all duration-200 flex items-center justify-center border-0 active:scale-95 relative overflow-visible
      ...
      `, style: {
            ...
            position: 'relative',
            zIndex: 1,
        }, onClick: handleMicClick, "aria-label": isListening ? 'Stop listening' : 'Start listening', type: "button", disabled: !isSupported, children: _jsx("span", { className: `text-4xl transition-colors duration-200 ${isListening ? 'text-red-500' : ''}`, children: "\uD83C\uDFA4" }) }));
}
