...
export default function CommandButtons({ message, setMessage }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("div", { className: "grid grid-cols-4 gap-2 mt-2", style: { width: '80vw', margin: '0 auto', marginTop: '25px', marginBottom: '30px' }, children: commands.map(cmd => (_jsx("button", { onClick: () => handleCommand(cmd), disabled: cmd === 'UNDO' && historyIndex <= 0, onMouseDown: () => setActiveButton(cmd), onMouseUp: () => setActiveButton(null), onMouseLeave: () => setActiveButton(null), className: `
          ...
          `, style: {
                        ...
                        position: 'relative',
                        zIndex: 1,
                    }, children: cmd }, cmd))) }), notification && (_jsx(StyledNotification, { message: notification.message, type: notification.type, onClose: () => setNotification(null) }))] }));
}
