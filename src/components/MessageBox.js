...
export default function MessageBox({ value, onChange, onTranslateClick, onRewriteClick, onAskAI, uploadedImage, setUploadedImage, onShowImageChoice, onGalleryUpload, onCameraCapture, language = 'en-US', onLanguageChange }) {
    ...
    const getTextboxBounds = () => {
        const textboxElement = document.querySelector('[style*="width: 85vw"]');
        if (textboxElement) {
    ...
    const requiresRealTimeSearch = (question) => {
    ...
    const processImageAndTextWithAI = async (question, imageData) => {
        ...
        apiUsageTracker.trackOpenAIUsage('chat/completions', 'gpt-4o', inputTokens, outputTokens, 'Image + Text Q&A', true);
        return aiResponse;
    };
    ...
    React.useEffect(() => {
        if (onAskAI) {
            ...
            window.triggerAskAI = () => handleAskAI(value);
        }
    }, [onAskAI, value]);
...
Text to correct: "${value}"`;
            ...
            apiUsageTracker.trackOpenAIUsage('chat/completions', 'gpt-4o-mini', inputTokens, outputTokens, 'Grammar Fix', true);
            ...
            apiUsageTracker.trackOpenAIUsage('chat/completions', 'gpt-4o-mini', 0, 0, 'Grammar Fix', false, errorMessage);
    ...
    const handleTextSelect = (selected, actions) => {
    ...
    return (_jsxs("div", { className: "textbox-container photo-frame-3d rounded-2xl overflow-hidden", style: {
            height: '210px',
            width: '85vw',
            margin: '0 auto'
                            ...
                            selectionMode: 'custom'
                        } }), _jsxs("div", { className: "absolute bottom-0 left-0 right-0 z-10 flex flex-row justify-between gap-4 py-2 px-4", style: { height: 'auto' }, children: [_jsx(LanguageToggleButton, { currentLanguage: language, onLanguageChange: onLanguageChange || handleLanguageChange, size: "sm", variant: "primary", className: "shadow-lg glassy-btn neon-grid-btn" }), _jsx("button", { onClick: handleMicClick, className: "w-8 h-8 glassy-btn neon-grid-btn rounded-full border-0 flex items-center justify-center text-xs font-bold transition-all duration-200 shadow-lg active:scale-95 relative overflow-visible", style: {
                                    ...
                                    width: '32px',
                                    height: '28px'
                                }, title: "Speech to text", children: isListening ? '🛑' : '🎤' }), _jsx(TextToSpeechButton, { onSpeak: handleTTS, text: value || " ", language: language, size: "sm", variant: "primary", className: "shadow-lg glassy-btn neon-grid-btn" }), _jsx("button", { onClick: handleCorrectGrammar, disabled: isCorrecting, className: "w-8 h-8 glassy-btn neon-grid-btn rounded-full border-0 flex items-center justify-center text-xs font-bold transition-all duration-200 shadow-lg active:scale-95 relative overflow-visible disabled:opacity-50 disabled:cursor-not-allowed", style: {
                                    ...
                                    width: '32px',
                                    height: '28px'
                                }, title: "Fix grammar, punctuation, and capitalization", children: isCorrecting ? '...' : 'Fix' }), _jsx("button", { onClick: handleImageUpload, className: "w-8 h-8 glassy-btn neon-grid-btn rounded-full border-0 flex items-center justify-center text-xs font-bold transition-all duration-200 shadow-lg active:scale-95 relative overflow-visible", style: {
                                    ...
                                    width: '32px',
                                    height: '28px'
                                }, title: "Upload image from gallery", children: "\uD83D\uDCF7" }), _jsx("button", { onClick: handleSaveComment, className: "w-8 h-8 glassy-btn neon-grid-btn rounded-full border-0 flex items-center justify-center text-xs font-bold transition-all duration-200 shadow-lg active:scale-95 relative overflow-visible", style: {
                                    ...
                                    width: '32px',
                                    height: '28px'
                                }, title: "Send message", children: "\uD83D\uDCE4" })] }), _jsx("input", { ref: fileInputRef, type: "file", accept: "image/*", onChange: handleFileChange, style: { display: 'none' } }), _jsx("input", { ref: cameraInputRef, type: "file", accept: "image/*", capture: "environment", onChange: handleFileChange, style: { display: 'none' } }), uploadedImage && (_jsx("div", { className: "mt-3 p-3 rounded-lg", onMouseDown: (e) => {
                        ...
                        }, children: _jsxs("div", { className: "relative w-full h-full", children: [_jsx("img", { src: uploadedImage.previewUrl, alt: "Uploaded preview", className: "object-cover rounded border border-white/20", style: {
                                        width: '61px',
                                        height: '61px',
                                        boxShadow: '0 2px 4px rgba(0, 0, 0, 0.3)',
                                        filter: 'drop-shadow(0 0 2px rgba(255, 255, 255, 0.2))'
                                    } }), _jsx("button", { onClick: handleRemoveImage, className: "absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500/80 hover:bg-red-500 text-white flex items-center justify-center transition-all duration-200 text-xs font-bold", style: {
                                        boxShadow: '0 1px 2px rgba(0, 0, 0, 0.3)',
                                        backdropFilter: 'blur(5px)'
                                    }, title: `Remove ${uploadedImage.file.name}`, children: "\u00D7" })] }) }))] }), isAskingAI && (_jsx("div", { className: "mt-4 p-4 rounded-lg text-center", style: {
                    backgroundColor: '#1a1a1a',
                    border: '1px solid #333',
                    color: '#fff'
                }, children: _jsxs("div", { className: "flex items-center justify-center gap-2", children: [_jsx("div", { className: "animate-spin w-4 h-4 border-2 border-blue-400 border-t-transparent rounded-full" }), _jsx("span", { className: "text-blue-400", children: "\uD83E\uDD16 AI is processing your question..." })] }) })), notification && (_jsx(StyledNotification, { message: notification.message, type: notification.type, onClose: () => setNotification(null) }))] }));
}
